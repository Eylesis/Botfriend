...
class Misc(commands.Cog):
    ...
    async def banana(self, ctx):
        ...
        if bananaStash == 0:
            await ctx.send('Why thank you {}! This banana is the first in my latest stash! I shall store it carefully.'.format(ctx.message.author.mention))
            self.bot.db.set_val('bananaStash', '1')
        else:
            bananaStash += 1
            self.bot.db.set_val('bananaStash', str(bananaStash))
            await ctx.send('Why thank you {}! I do so enjoy bananas, however I am not hungry at the moment, so I shall save this for later! My stash has {} bananas in it, can you believe it?'
